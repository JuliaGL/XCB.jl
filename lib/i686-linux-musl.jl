...
function Base.getproperty(x::Ptr{xcb_xkb_get_kbd_by_name_replies_t}, f::Symbol)
    ...
    f === :compat_map && return Ptr{__JL_Ctag_19}(x + 76)
    f === :indicator_maps && return Ptr{__JL_Ctag_20}(x + 116)
    f === :key_names && return Ptr{__JL_Ctag_21}(x + 152)
    f === :geometry && return Ptr{__JL_Ctag_22}(x + 244)
    return getfield(x, f)
end

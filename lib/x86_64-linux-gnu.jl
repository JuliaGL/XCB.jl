...
function Base.getproperty(x::Ptr{xcb_xkb_get_kbd_by_name_replies_t}, f::Symbol)
    ...
    f === :compat_map && return Ptr{__JL_Ctag_32}(x + 112)
    f === :indicator_maps && return Ptr{__JL_Ctag_33}(x + 160)
    f === :key_names && return Ptr{__JL_Ctag_34}(x + 200)
    f === :geometry && return Ptr{__JL_Ctag_35}(x + 328)
    return getfield(x, f)
end
